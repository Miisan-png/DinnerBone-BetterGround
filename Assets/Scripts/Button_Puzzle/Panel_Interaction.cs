...
public class Panel_Interaction : MonoBehaviour, I_Interactable, IInteractionIdentifier
{
    ...
    [SerializeField] private float x_offset = 0f;
    ...
    void Start()
    {
        ...
        {
            if (power_components[i] != null && !supposedTargetRotation.ContainsKey(power_components[i]))
            {
    ...
    void OnValidate()
    {
        if (arrow_indicator != null && current_component_index < power_components.Length && power_components[current_component_index] != null)
        {
            ...
            arrow_indicator.transform.position = target_pos;
        }
    }
            ...
            if (power_indicator_planes[i] != null)
            {
                ...
                if (renderer != null)
                {
                    if (Application.isPlaying)
                        ...
                        indicator_materials[i] = renderer.sharedMaterial;

                    if (Application.isPlaying)
                        renderer.material = indicator_materials[i];
                    else
                        renderer.sharedMaterial = indicator_materials[i];
    ...
    private void SetIndicatorRed(int index)
    {
        ...
        indicator_materials[index].SetColor("_GlowColor", new Color(0.8f, 0.1f, 0.1f, 1f));
        indicator_materials[index].SetFloat("_GlowIntensity", 3f);
    }

    private void SetIndicatorGreen(int index)
    {
        if (indicator_materials == null || index >= indicator_materials.Length || indicator_materials[index] == null) return;
        indicator_materials[index].SetColor("_GlowColor", new Color(0.2f, 1f, 0.2f, 1f));
        indicator_materials[index].SetFloat("_GlowIntensity", 3f);
    }

    public bool Can_Interact(Player_Type player_type) => !is_puzzle_active && !puzzle_solved;
    ...
    private void HandleInteractInput()
    {
        ...
        {
            if (!is_rotating_object) StartObjectRotation();
            else ExitObjectRotation();
        }
    ...
    private void HandleNavigation()
    {
        ...
        if (has_input && !input_pressed)
        {
            ...
            if (input.z > 0.5f) new_index = MoveInGrid(-3);
            else if (input.z < -0.5f) new_index = MoveInGrid(3);
            else if (input.x < -0.5f) new_index = MoveInGrid(-1);
            else if (input.x > 0.5f) new_index = MoveInGrid(1);

            if (new_index != current_component_index)
            {
                if (has_been_rotated[old_index]) StartResetTimer(old_index);
                current_component_index = new_index;
                MoveArrowToCurrentComponent();
                UpdateArrowAppearance();
            }
        }
        else if (!has_input) input_pressed = false;
    }
    ...
    private void HandleObjectRotation()
    {
        ...
        {
            input_pressed = true;
            if (input.x < -0.5f) RotateCurrentObject(rotation_speed);
            else if (input.x > 0.5f) RotateCurrentObject(-rotation_speed);
        }
        else if (!has_input) input_pressed = false;
    }
    ...
    {
        if (current_component_index < power_components.Length && power_components[current_component_index] != null)
        {
            ...
            if (reset_tweens[current_component_index] != null) reset_tweens[current_component_index].Kill();
            target.DORotate(targetRotation, rotation_duration)
                .SetEase(Ease.OutQuad)
                .OnComplete(() => { CheckComponentCorrectness(current_component_index); });
        }
    }
    ...
    private void CheckComponentCorrectness(int componentIndex)
    {
        ...
        if (is_correct && !was_correct)
        {
            ...
            if (reset_tweens[componentIndex] != null) reset_tweens[componentIndex].Kill();
        }
        else if (!is_correct && was_correct)
        {
            correct_components_count--;
            if (is_dual_panel_mode && dual_panel_manager != null) dual_panel_manager.NotifyPanelComplete(panel_id, false);
        }
        if (correct_components_count == power_components.Length) CompletePuzzle();
    }
    ...
    private void StartResetTimer(int componentIndex)
    {
        ...
        reset_tweens[componentIndex] = DOVirtual.DelayedCall(reset_delay, () => { ResetComponent(componentIndex); });
    }
    ...
    private void ResetComponent(int componentIndex)
    {
        ...
        if (was_correct) correct_components_count--;
    }
    ...
    private void UpdatePowerIndicators()
    {
        ...
        {
            int index = i;
            DOVirtual.DelayedCall(i * 0.3f, () => { SetIndicatorGreen(index); });
        }
    }
    ...
    private void CompletePuzzle()
    {
        ...
        for (int i = 0; i < reset_tweens.Length; i++)
            if (reset_tweens[i] != null) reset_tweens[i].Kill();
        ...
        else if (!is_dual_panel_mode)
        {
            ...
            DOVirtual.DelayedCall(2f, () => { ExitPuzzle(); });
        }
    }
    ...
    private void UpdateArrowAppearance()
    {
        ...
        if (arrowRenderer != null && arrowRenderer.material != null)
        {
            ...
            Color targetGlowColor = is_rotating_object ? Color.green : Color.yellow;
            if (mat.HasProperty("_GlowColor")) mat.DOColor(targetGlowColor, "_GlowColor", 0.2f);
        }
    }
    ...
    private int MoveInGrid(int direction)
    {
        ...
        if (new_index < 0 || new_index >= power_components.Length) return current_component_index;
        return new_index;
    }
    ...
    {
        if (interacting_player.Get_Exit_Input()) ExitPuzzle();
    }
    ...
    private void ExitPuzzle()
    {
        ...
        is_rotating_object = false;
        if (arrow_indicator != null) arrow_indicator.SetActive(false);
        if (player_movement != null) player_movement.enabled = true;
        interacting_player = null;
    }

    public string Get_Interaction_Text() => puzzle_solved ? "Power restored" : "Use power panel";
    public Vector3 Get_Interaction_Position() => transform.position;
    public string GetInteractionID() => interaction_id;
    public void SetObjectRotatable(int index, bool canRotate) { if (index >= 0 && index < can_rotate_objects.Length) can_rotate_objects[index] = canRotate; }
    public bool IsRotatingObject() => is_rotating_object;
    public int GetCurrentComponentIndex() => current_component_index;
    public void SetDualPanelManager(Dual_Panel_Manager manager) { dual_panel_manager = manager; is_dual_panel_mode = true; }
    public void SetPanelID(int id) { panel_id = id; }
    public void TriggerPanelCompletion() { UpdatePowerIndicators(); }
    public void ExitPuzzleFromManager() { ExitPuzzle(); }
    public void ForceReset()
    {
        puzzle_solved = false;
        correct_components_count = 0;
        for (int i = 0; i < components_correct.Length; i++) components_correct[i] = false;
        for (int i = 0; i < has_been_rotated.Length; i++) has_been_rotated[i] = false;
        for (int i = 0; i < reset_tweens.Length; i++) { if (reset_tweens[i] != null) reset_tweens[i].Kill(); }
        for (int i = 0; i < power_components.Length; i++) { if (power_components[i] != null) power_components[i].DORotate(original_rotations[i], reset_duration); }
        for (int i = 0; i < power_indicator_planes.Length; i++) SetIndicatorRed(i);
    }
    public bool IsPuzzleSolved() => puzzle_solved;
}
