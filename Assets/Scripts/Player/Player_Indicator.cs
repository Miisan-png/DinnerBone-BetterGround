...
public class Player_Indicator : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (shadowObject != null)
            shadowObject.SetActive(!isJumping);
    }

    private void CheckJumpingStatus()
    {
        if (characterController != null)
            isJumping = characterController.velocity.y > 0.5f || !characterController.isGrounded;
        else
            isJumping = false;
    }
    ...
    private void CheckMovementStatus()
    {
        if (playerMovement != null)
        {
            ...
            if (isMoving) idleTimer = 0f;
            else if (wasMoving && !isMoving) idleTimer = 0f;
            else if (!isMoving) idleTimer += Time.deltaTime;
    ...
    public bool IsUsingPlayerMovementScript() => playerMovement != null;
