...
public class SpiderLegEvents : MonoBehaviour
{
    ...
    public void PlayImpactVFX()
    {
        ...
        if (CameraUtils.IsPointVisible(Camera.main, impactPoint.position))
        {
            SoundSetting soundSetting = new SoundSetting()
            {
                Volume = 0.75f
            };
            SoundManager.Instance.PlaySound(new SoundVariationizer("sfx_spider_attack", 0.1f), null, soundSetting);
        }
        else
        {
