	...
	LOD 100
		...
		Pass
		{
			...
			v2f vert ( appdata v )
			{
				...
				float Dist41_g1848 = distance( ( POSITION653 * _Vector0 ) , ( _Vector0 * _WorldSpaceCameraPos ) );
				float vertexToFrag49_g1848 = ( saturate( ( 1.0 - ( ( Dist41_g1848 - _FarFade ) / _FarTransition ) ) ) * saturate( ( ( Dist41_g1848 - _CloseFade ) / _CloseTransition ) ) );
				o.ase_texcoord3.w = vertexToFrag49_g1848;
				float3 ase_worldPos = mul(unity_ObjectToWorld, float4( (v.vertex).xyz, 1 )).xyz;
				float3 worldSpaceLightDir = UnityWorldSpaceLightDir(ase_worldPos);
				float dotResult3_g1847 = dot( -worldSpaceLightDir , float3( 0,1,0 ) );
				float vertexToFrag9_g1847 = saturate( ( dotResult3_g1847 * 4.0 ) );
				o.ase_texcoord4.x = vertexToFrag9_g1847;
				#ifdef _PARTICLEMODE_ON
				...
				#endif
				float vertexToFrag32_g1916 = ( ( _HaloSize * ( FlickerSize477 * staticSwitch4535 ) ) * 0.5 );
				o.ase_texcoord4.y = vertexToFrag32_g1916;
				float3 pos75_g1916 = POSITION653;
				float vertexToFrag15_g1919 = ( unity_OrthoParams.w <= 0.0 ? ( distance( _WorldSpaceCameraPos , pos75_g1916 ) / -UNITY_MATRIX_P[ 1 ][ 1 ] ) : unity_OrthoParams.y );
				o.ase_texcoord4.z = vertexToFrag15_g1919;
				...
				float dotResult31_g1916 = dot( ase_viewDirSafeWS , ( _WorldSpaceCameraPos - pos75_g1916 ) );
				float vertexToFrag36_g1916 = step( 0.0 , dotResult31_g1916 );
				o.ase_texcoord4.w = vertexToFrag36_g1916;
				float vertexToFrag62_g1916 = distance( _WorldSpaceCameraPos , pos75_g1916 );
				o.ase_texcoord5.x = vertexToFrag62_g1916;
			...
			fixed4 frag (v2f i ) : SV_Target
			{
				...
				float eyeDepth6_g1845 = LinearEyeDepth(SAMPLE_DEPTH_TEXTURE( _CameraDepthTexture, ase_screenPosNorm.xy ));
				float3 ase_viewVectorWS = ( _WorldSpaceCameraPos.xyz - WorldPosition );
				float3 ase_viewDirWS = normalize( ase_viewVectorWS );
				float4x4 invertVal5_g1846 = Inverse4x4( UNITY_MATRIX_M );
				float dotResult4_g1845 = dot( ase_viewDirWS , -mul( UNITY_MATRIX_M, float4( (transpose( mul( invertVal5_g1846, UNITY_MATRIX_I_V ) )[2]).xyz , 0.0 ) ).xyz );
				float3 worldToView72_g1845 = mul( UNITY_MATRIX_V, float4( WorldPosition, 1 ) ).xyz;
				float clampDepth65_g1845 = SAMPLE_DEPTH_TEXTURE( _CameraDepthTexture, ase_screenPosNorm.xy );
				#ifdef UNITY_REVERSED_Z
				float staticSwitch68_g1845 = ( 1.0 - clampDepth65_g1845 );
				#else
				float staticSwitch68_g1845 = clampDepth65_g1845;
				#endif
				...
				float3 viewToWorld74_g1845 = mul( UNITY_MATRIX_I_V, float4( appendResult73_g1845, 1 ) ).xyz;
				float3 ReconstructedPos539 = ( unity_OrthoParams.w < 1.0 ? ( ( eyeDepth6_g1845 * ( ase_viewDirWS / dotResult4_g1845 ) ) + _WorldSpaceCameraPos ) : viewToWorld74_g1845 );
				float3 ase_objectPosition = UNITY_MATRIX_M._m03_m13_m23;
				#ifdef _PARTICLEMODE_ON
				...
				float3 UVS73_g1912 = ( ( ReconstructedPos539 * 0.1 ) * _NoiseScale );
				...
				float3 viewToWorldDir4807 = ASESafeNormalize( mul( UNITY_MATRIX_I_V, float4( decodeViewNormalStereo4806, 0 ) ).xyz );
				float3 worldNormals1927 = viewToWorldDir4807;
				float3 temp_cast_3 = (3.0).xxx;
				float3 temp_output_141_0_g1912 = pow( abs( worldNormals1927 ) , temp_cast_3 );
				float3 temp_cast_4 = (1.0).xxx;
				float dotResult144_g1912 = dot( temp_output_141_0_g1912 , temp_cast_4 );
				float3 break147_g1912 = ( saturate( temp_output_141_0_g1912 ) / dotResult144_g1912 );
				float2 clipScreen151_g1912 = abs( ase_screenPosNorm.xy ) * _ScreenParams.xy;
				float dither151_g1912 = Dither8x8Bayer( fmod(clipScreen151_g1912.x, 8), fmod(clipScreen151_g1912.y, 8) );
				float2 lerpResult19_g1912 = lerp( (UVS73_g1912).xz , ( (UVS73_g1912).yz * 0.9 ) , round( ( ( ( ( 1.0 - break147_g1912.x ) * break147_g1912.x ) * dither151_g1912 ) + break147_g1912.x ) ));
				float2 lerpResult20_g1912 = lerp( lerpResult19_g1912 , ( (UVS73_g1912).xy * 0.94 ) , round( ( break147_g1912.z + ( dither151_g1912 * ( break147_g1912.z * ( 1.0 - break147_g1912.z ) ) ) ) ));
				float temp_output_60_0_g1912 = RANDOMNESS3727;
				float mulTime41_g1912 = _Time.y * ( ( _NoiseMovement + ( temp_output_60_0_g1912 * 0.1 ) ) * 0.2 );
				float Time70_g1912 = ( mulTime41_g1912 + ( temp_output_60_0_g1912 * UNITY_PI ) );
				float4 tex2DNode3_g1912 = SAMPLE_TEXTURE2D( _NoiseTexture, sampler_NoiseTexture, ( lerpResult20_g1912 + ( Time70_g1912 * float2( 1.02,0.87 ) ) ) );
				#if defined( _TEXTUREPACKING_RED )
				float staticSwitch211_g1912 = tex2DNode3_g1912.r;
				#elif defined( _TEXTUREPACKING_REDXGREEN )
				float staticSwitch211_g1912 = tex2DNode3_g1912.g;
				#elif defined( _TEXTUREPACKING_ALPHA )
				float staticSwitch211_g1912 = tex2DNode3_g1912.a;
				#else
				float staticSwitch211_g1912 = tex2DNode3_g1912.r;
				#endif
				float4 tex2DNode14_g1912 = SAMPLE_TEXTURE2D( _NoiseTexture, sampler_NoiseTexture, ( ( lerpResult20_g1912 * 0.7 ) + ( Time70_g1912 * float2( -0.72,-0.67 ) ) ) );
				#if defined( _TEXTUREPACKING_RED )
				float staticSwitch212_g1912 = tex2DNode14_g1912.r;
				#elif defined( _TEXTUREPACKING_REDXGREEN )
				float staticSwitch212_g1912 = tex2DNode14_g1912.r;
				#elif defined( _TEXTUREPACKING_ALPHA )
				float staticSwitch212_g1912 = tex2DNode14_g1912.a;
				#else
				float staticSwitch212_g1912 = tex2DNode14_g1912.r;
				#endif
				#ifdef ___NOISE____ON
				float staticSwitch186_g1912 = ( 1.0 + ( ( staticSwitch211_g1912 * staticSwitch212_g1912 * _Noisiness ) - ( _Noisiness * 0.2 ) ) );
				#else
				float staticSwitch186_g1912 = 1.0;
				#endif
				float noise3969 = staticSwitch186_g1912;
				...
				float temp_output_8_0_g1914 = lPosterize4215;
				float BrightnessBoost3902 = saturate( pow( temp_output_3981_0 , 30.0 ) );
				float temp_output_9_0_g1914 = ( smoothstepResult745 + BrightnessBoost3902 );
				float temp_output_5_0_g1914 = ( 256.0 / temp_output_8_0_g1914 );
				float GradientMask555 = ( smoothstepResult745 * ( temp_output_8_0_g1914 <= 0.0 ? temp_output_9_0_g1914 : saturate( ( floor( ( temp_output_9_0_g1914 * temp_output_5_0_g1914 ) ) / temp_output_5_0_g1914 ) ) ) );
				float2 temp_cast_5 = (( 1.0 - GradientMask555 )).xx;
				float4 temp_output_2_0_g1915 = ( SAMPLE_TEXTURE2D( _GradientTexture, sampler_GradientTexture, temp_cast_5 ) * _LightTint * i.ase_color );
				float temp_output_8_0_g1913 = lPosterize4215;
				...
				float3 pos338_g1910 = POSITION653;
				float rad280_g1910 = ( SCALE3835 * 0.5 );
				float temp_output_298_0_g1910 = ( 1.0 - ( abs( (pos338_g1910).y ) / rad280_g1910 ) );
				float3 normalizeResult278_g1910 = normalize( ( pos338_g1910 - _WorldSpaceCameraPos ) );
				float4x4 invertVal5_g1911 = Inverse4x4( UNITY_MATRIX_M );
				float dotResult283_g1910 = dot( normalizeResult278_g1910 , -mul( UNITY_MATRIX_M, float4( (transpose( mul( invertVal5_g1911, UNITY_MATRIX_I_V ) )[2]).xyz , 0.0 ) ).xyz );
				float temp_output_310_0_g1910 = ( temp_output_298_0_g1910 * dotResult283_g1910 );
				float4 worldToScreen279_g1910 = ComputeScreenPos( mul(UNITY_MATRIX_VP, float4(( pos338_g1910 * float3(1,0.1,1) ), 1.0)) );
				float3 worldToScreen279_g1910NDC = worldToScreen279_g1910/worldToScreen279_g1910.w;
				float2 temp_output_344_0_g1910 = (( float4( worldToScreen279_g1910NDC , 0.0 ) - ase_screenPosNorm )).xy;
				float2 temp_output_384_0_g1910 = ( temp_output_310_0_g1910 * temp_output_344_0_g1910 );
				float2 lightDirScreen311_g1910 = temp_output_384_0_g1910;
				float temp_output_476_0_g1910 = ( _ShadowThreshold * 0.01 );
				float threshold311_g1910 = temp_output_476_0_g1910;
				float stepsSpace311_g1910 = 1.0;
				#if defined( _SCREENSHADOWS_OFF )
				float staticSwitch296_g1910 = -1.0;
				#elif defined( _SCREENSHADOWS_LOW )
				float staticSwitch296_g1910 = 16.0;
				#elif defined( _SCREENSHADOWS_MEDIUM )
				float staticSwitch296_g1910 = 32.0;
				#elif defined( _SCREENSHADOWS_HIGH )
				float staticSwitch296_g1910 = 64.0;
				#elif defined( _SCREENSHADOWS_INSANE )
				float staticSwitch296_g1910 = 128.0;
				#else
				float staticSwitch296_g1910 = -1.0;
				#endif
				float StepQual300_g1910 = staticSwitch296_g1910;
				float stepsNum311_g1910 = StepQual300_g1910;
				float radius311_g1910 = rad280_g1910;
				float vertexToFrag49_g1848 = i.ase_texcoord3.w;
				float vertexToFrag9_g1847 = i.ase_texcoord4.x;
				#ifdef _DAYFADING_ON
				float staticSwitch11_g1847 = vertexToFrag9_g1847;
				#else
				float staticSwitch11_g1847 = 1.0;
				#endif
				float DistanceFade3571 = ( (( DistanceFade )?( vertexToFrag49_g1848 ):( 1.0 )) * staticSwitch11_g1847 );
				float temp_output_335_0_g1910 = DistanceFade3571;
				float mask311_g1910 = temp_output_335_0_g1910;
				float3 worldPos353_g1910 = ReconstructedPos539;
				float3 wPos311_g1910 = worldPos353_g1910;
				float3 lightPos311_g1910 = pos338_g1910;
				float3 camPos311_g1910 = _WorldSpaceCameraPos;
				float2 screenPos311_g1910 = ase_screenPosNorm.xy;
				float3 offsetDir485_g1910 = ( dotResult283_g1910 * normalizeResult278_g1910 );
				float3 offsetDir311_g1910 = offsetDir485_g1910;
				float localExperimentalScreenShadowsBirp311_g1910 = ExperimentalScreenShadowsBirp( lightDirScreen311_g1910 , threshold311_g1910 , stepsSpace311_g1910 , stepsNum311_g1910 , radius311_g1910 , mask311_g1910 , wPos311_g1910 , lightPos311_g1910 , camPos311_g1910 , screenPos311_g1910 , offsetDir311_g1910 );
				#if defined( _SCREENSHADOWS_OFF )
				float staticSwitch341_g1910 = 1.0;
				#elif defined( _SCREENSHADOWS_LOW )
				float staticSwitch341_g1910 = localExperimentalScreenShadowsBirp311_g1910;
				#elif defined( _SCREENSHADOWS_MEDIUM )
				float staticSwitch341_g1910 = localExperimentalScreenShadowsBirp311_g1910;
				#elif defined( _SCREENSHADOWS_HIGH )
				float staticSwitch341_g1910 = localExperimentalScreenShadowsBirp311_g1910;
				...
				float staticSwitch341_g1910 = 1.0;
				#endif
				float ScreenSpaceShadows1881 = staticSwitch341_g1910;
				float smoothstepResult4598 = smoothstep( 0.0 , _ShadingSoftness , saturate( ( dotResult436 * noise3969 * ScreenSpaceShadows1881 ) ));
				float temp_output_9_0_g1913 = smoothstepResult4598;
				float temp_output_5_0_g1913 = ( 256.0 / temp_output_8_0_g1913 );
				float ShadingMask552 = saturate( ( ( temp_output_8_0_g1913 <= 0.0 ? temp_output_9_0_g1913 : saturate( ( floor( ( temp_output_9_0_g1913 * temp_output_5_0_g1913 ) ) / temp_output_5_0_g1913 ) ) ) + _ShadingBlend ) );
				...
				float vertexToFrag32_g1916 = i.ase_texcoord4.y;
				float3 pos75_g1916 = POSITION653;
				float4 worldToScreen9_g1916 = ComputeScreenPos( mul(UNITY_MATRIX_VP, float4(pos75_g1916, 1.0)) );
				float3 worldToScreen9_g1916NDC = worldToScreen9_g1916/worldToScreen9_g1916.w;
				float2 appendResult15_g1916 = (float2(( _ScreenParams.x / _ScreenParams.y ) , 1.0));
				float vertexToFrag15_g1919 = i.ase_texcoord4.z;
				float smoothstepResult33_g1916 = smoothstep( 0.0 , vertexToFrag32_g1916 , length( ( ( ( (worldToScreen9_g1916NDC).xy - ScreenPos4516 ) * appendResult15_g1916 ) * vertexToFrag15_g1919 ) ));
				float vertexToFrag36_g1916 = i.ase_texcoord4.w;
				float HaloMask38_g1916 = ( ( 1.0 - smoothstepResult33_g1916 ) * vertexToFrag36_g1916 );
				float temp_output_8_0_g1917 = _HaloPosterize;
				float temp_output_9_0_g1917 = HaloMask38_g1916;
				float temp_output_5_0_g1917 = ( 256.0 / temp_output_8_0_g1917 );
				float HaloPosterized43_g1916 = ( HaloMask38_g1916 * ( temp_output_8_0_g1917 <= 0.0 ? temp_output_9_0_g1917 : saturate( ( floor( ( temp_output_9_0_g1917 * temp_output_5_0_g1917 ) ) / temp_output_5_0_g1917 ) ) ) );
				float2 temp_cast_11 = (( 1.0 - HaloPosterized43_g1916 )).xx;
				float4 temp_output_2_0_g1918 = ( SAMPLE_TEXTURE2D( _GradientTexture, sampler_GradientTexture, temp_cast_11 ) * _HaloTint * i.ase_color );
				float vertexToFrag62_g1916 = i.ase_texcoord5.x;
				float HaloPenetrationMask68_g1916 = saturate( pow( saturate( ( distance( ReconstructedPos539 , _WorldSpaceCameraPos ) - vertexToFrag62_g1916 ) ) , _HaloDepthFade ) );
				#ifdef ___HALO____ON
				float3 staticSwitch54_g1916 = ( (temp_output_2_0_g1918).rgb * ( (temp_output_2_0_g1918).a * HaloMask38_g1916 * HaloPenetrationMask68_g1916 * HaloPosterized43_g1916 ) );
				#else
				float3 staticSwitch54_g1916 = temp_cast_10;
				#endif
				float3 halo4548 = staticSwitch54_g1916;
				float3 vertexToFrag67_g1436 = i.ase_texcoord5.yzw;
				float3 FlickerHue1892 = vertexToFrag67_g1436;
				float3 temp_output_17_0_g1920 = ( ( ( ( ( (temp_output_2_0_g1915).rgb * ( (temp_output_2_0_g1915).a * FinalLightMask4298 * 0.1 ) ) + Spec4287 ) + halo4548 ) * FlickerHue1892 ) * ( DistanceFade3571 * FlickerAlpha416 ) );
				float2 clipScreen10_g1920 = abs( ase_screenPosNorm.xy ) * _ScreenParams.xy;
				float dither10_g1920 = Dither4x4Bayer( fmod(clipScreen10_g1920.x, 4), fmod(clipScreen10_g1920.y, 4) );
				float3 break3_g1920 = temp_output_17_0_g1920;
				float smoothstepResult9_g1920 = smoothstep( 0.0 , _DitherIntensity , ( 0.333 * ( break3_g1920.x + break3_g1920.y + break3_g1920.z ) * ( _DitherIntensity + 1.0 ) ));
				dither10_g1920 = step( dither10_g1920, saturate( smoothstepResult9_g1920 * 1.00001 ) );
				#ifdef _DITHERINGPATTERN_ON
				float3 staticSwitch12_g1920 = ( temp_output_17_0_g1920 * dither10_g1920 );
				#else
				float3 staticSwitch12_g1920 = temp_output_17_0_g1920;
				#endif
				float3 temp_output_4553_0 = staticSwitch12_g1920;
				...
				float3 staticSwitch4585 = temp_output_4553_0;
				#endif
				float4 appendResult3578 = (float4(staticSwitch4585 , 1.0));
...
Node;AmplifyShaderEditor.ScreenPosInputsNode;4514;-4622.879,-73.00718;Float;False;0;False;0;5;FLOAT4;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
...
Node;AmplifyShaderEditor.CommentaryNode;2273;-5100.651,152.9973;Inherit;False;1045.893;286.6611;;7;1927;4807;4806;4805;4518;2275;4939;NormalsTexture;0.5424528,1,0.8822392,1;0;0
...
Node;AmplifyShaderEditor.GetLocalVarNode;4518;-5050.706,244.8076;Inherit;False;4516;ScreenPos;1;0;OBJECT;;False;1;FLOAT2;0
...
Node;AmplifyShaderEditor.FunctionNode;4569;-3627.232,248.7301;Inherit;False;DayAlpha;46;;1847;bc1f8ebe2e26696419e0099f8a3e27dc;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.FunctionNode;4792;-3808.967,56.07874;Inherit;False;AdvancedCameraFade;12;;1848;e6e830f789d28b746963801d61c2a1ec;0;6;40;FLOAT;0;False;46;FLOAT;0;False;47;FLOAT;0;False;48;FLOAT;0;False;17;FLOAT3;0,0,0;False;20;FLOAT3;0,0,0;False;1;FLOAT;0
...
Node;AmplifyShaderEditor.DecodeViewNormalStereoHlpNode;4806;-4742.688,241.6837;Inherit;False;1;0;FLOAT4;0,0,0,0;False;1;FLOAT3;0
...
Node;AmplifyShaderEditor.TransformDirectionNode;4807;-4480.865,241.3839;Inherit;False;View;World;True;Fast;True;1;0;FLOAT3;0,0,0;False;4;FLOAT3;0;FLOAT;1;FLOAT;2;FLOAT;3
...
Node;AmplifyShaderEditor.RegisterLocalVarNode;1927;-4284.113,243.5302;Inherit;False;worldNormals;-1;True;1;0;FLOAT3;0,0,0;False;1;FLOAT3;0
Node;AmplifyShaderEditor.GetLocalVarNode;4234;-2473.791,-472.0054;Inherit;False;4107;LocalPos;1;0;OBJECT;;False;1;FLOAT3;0
...
Node;AmplifyShaderEditor.NegateNode;4235;-2304.88,-472.5057;Inherit;False;1;0;FLOAT3;0,0,0;False;1;FLOAT3;0
...
Node;AmplifyShaderEditor.FunctionNode;5010;-2811.362,115.2712;Inherit;False;ExperimentalScreenSpaceShadows;40;;1910;79f826106fc5f154c96059cc1326b755;0;4;337;FLOAT3;0,0,0;False;336;FLOAT3;0,0,0;False;370;FLOAT;0;False;335;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.NormalizeNode;3808;-2124.026,-473.2162;Inherit;False;False;1;0;FLOAT3;0,0,0;False;1;FLOAT3;0
Node;AmplifyShaderEditor.FunctionNode;4619;-3084.65,-435.7374;Inherit;False;3DNoiseMap;25;;1912;2fca756491ec7bf4e9c71d18280c45cc;0;5;257;FLOAT3;0,0,0;False;21;FLOAT3;0,0,0;False;1;FLOAT3;0,0,0;False;60;FLOAT;0;False;229;FLOAT2;0,0;False;2;FLOAT;0;FLOAT;213
...
Node;AmplifyShaderEditor.RegisterLocalVarNode;4264;-1983.805,-473.4308;Inherit;False;LightDir;-1;True;1;0;FLOAT3;0,0,0;False;1;FLOAT3;0
Node;AmplifyShaderEditor.GetLocalVarNode;2274;-2071.707,-400.0095;Inherit;False;1927;worldNormals;1;0;OBJECT;;False;1;FLOAT3;0
...
Node;AmplifyShaderEditor.DotProductOpNode;436;-1785.206,-473.019;Inherit;False;2;0;FLOAT3;0,0,0;False;1;FLOAT3;0,0,0;False;1;FLOAT;0
...
Node;AmplifyShaderEditor.FunctionNode;4213;-1042.616,-472.2397;Inherit;False;SimplePosterize;-1;;1913;163fbd1f7d6893e4ead4288913aedc26;0;2;9;FLOAT;0;False;8;FLOAT;0;False;1;FLOAT;0
...
Node;AmplifyShaderEditor.FunctionNode;4203;1440.282,-490.2576;Inherit;False;SimplePosterize;-1;;1914;163fbd1f7d6893e4ead4288913aedc26;0;2;9;FLOAT;0;False;8;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.StepOpNode;485;1328.647,-900.0927;Inherit;False;2;0;FLOAT;0.01;False;1;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.GetLocalVarNode;4236;-2034.173,90.58859;Inherit;False;4264;LightDir;1;0;OBJECT;;False;1;FLOAT3;0
...
Node;AmplifyShaderEditor.SimpleAddOpNode;4266;-1821.406,-24.03392;Inherit;False;2;2;0;FLOAT3;0,0,0;False;1;FLOAT3;0,0,0;False;1;FLOAT3;0
Node;AmplifyShaderEditor.GetLocalVarNode;4268;-1831.354,76.26473;Inherit;False;1927;worldNormals;1;0;OBJECT;;False;1;FLOAT3;0
...
Node;AmplifyShaderEditor.NormalizeNode;4267;-1650.484,-22.0349;Inherit;False;False;1;0;FLOAT3;0,0,0;False;1;FLOAT3;0
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;4275;-1627.239,74.07735;Inherit;False;2;2;0;FLOAT3;0,0,0;False;1;FLOAT3;0,0,0;False;1;FLOAT3;0
...
Node;AmplifyShaderEditor.DotProductOpNode;4231;-1461.507,41.2541;Inherit;False;2;0;FLOAT3;0,0,0;False;1;FLOAT3;0,0,0;False;1;FLOAT;0
...
Node;AmplifyShaderEditor.RegisterLocalVarNode;4298;2489.125,-550.713;Inherit;False;FinalLightMask;-1;True;1;0;FLOAT;0;False;1;FLOAT;0
...
Node;AmplifyShaderEditor.FunctionNode;4810;611.8979,-81.23064;Inherit;False;Alpha Split;-1;;1915;07dab7960105b86429ac8eebd729ed6d;0;1;2;COLOR;0,0,0,0;False;2;FLOAT3;0;FLOAT;6
...
Node;AmplifyShaderEditor.FunctionNode;4811;501.7485,-425.7458;Inherit;False;HaloFunction;6;;1916;739bbcda129bbae47870a33d01fe91b1;0;5;69;FLOAT3;0,0,0;False;70;FLOAT3;0,0,0;False;72;FLOAT2;0,0;False;74;FLOAT;0;False;80;SAMPLER2D;;False;1;FLOAT3;0
Node;AmplifyShaderEditor.CommentaryNode;3841;1301.24,-130.634;Inherit;False;1146.034;484.6983;Final Mix;12;4793;4520;2676;4553;3579;657;1901;600;1897;1384;3572;4549;;0.2959238,0.4695747,0.6603774,1;0;0
...
Node;AmplifyShaderEditor.GetLocalVarNode;4549;1574.824,2.40826;Inherit;False;4548;halo;1;0;OBJECT;;False;1;FLOAT3;0
...
Node;AmplifyShaderEditor.FunctionNode;4553;2248.181,-79.12321;Inherit;False;Dithering;36;;1920;b490ae982132eea449373f63bf44f108;0;1;17;FLOAT3;0,0,0;False;1;FLOAT3;0
Node;AmplifyShaderEditor.SaturateNode;4587;2465.867,16.16205;Inherit;False;1;0;FLOAT3;0,0,0;False;1;FLOAT3;0
Node;AmplifyShaderEditor.OneMinusNode;4572;2599.293,15.31504;Inherit;False;1;0;FLOAT3;0,0,0;False;1;FLOAT3;0
Node;AmplifyShaderEditor.CommentaryNode;4615;2991.885,100.4375;Inherit;False;297;283;BehindTheScene;3;4617;4616;4561;;1,1,1,1;0;0
Node;AmplifyShaderEditor.StaticSwitch;4585;2759.747,-78.52747;Inherit;False;Property;_Blendmode;Blendmode;48;0;Create;True;0;0;0;False;1;Space(15);False;0;0;0;True;;KeywordEnum;3;Additive;Contrast;Negative;Create;True;True;All;9;1;FLOAT3;0,0,0;False;0;FLOAT3;0,0,0;False;2;FLOAT3;0,0,0;False;3;FLOAT3;0,0,0;False;4;FLOAT3;0,0,0;False;5;FLOAT3;0,0,0;False;6;FLOAT3;0,0,0;False;7;FLOAT3;0,0,0;False;8;FLOAT3;0,0,0;False;1;FLOAT3;0
Node;AmplifyShaderEditor.RangedFloatNode;5013;2823.738,38.06876;Inherit;False;Constant;_Float3;Float 3;23;0;Create;True;0;0;0;False;0;False;1;0;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.ComponentMaskNode;4939;-4766.801,373.5373;Inherit;False;True;True;False;False;1;0;FLOAT4;0,0,0,0;False;1;FLOAT2;0
Node;AmplifyShaderEditor.FunctionNode;4938;-1383.981,-1223.522;Inherit;False;Reconstruct World Position From Depth;-1;;1921;e7094bcbcc80eb140b2a3dbe6a861de8;0;0;1;FLOAT4;0
Node;AmplifyShaderEditor.RangedFloatNode;4809;-2543.461,278.3967;Inherit;False;Constant;_Float0;Float 0;23;0;Create;True;0;0;0;False;0;False;1;0;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.GetLocalVarNode;4605;-3287.827,-512.5876;Inherit;False;4107;LocalPos;1;0;OBJECT;;False;1;FLOAT3;0
...
Node;AmplifyShaderEditor.RangedFloatNode;4616;3010.818,220.8691;Inherit;False;Property;_SrcBlend;SrcBlend;50;3;[HideInInspector];[IntRange];[Enum];Create;True;0;3;Default;0;Off;1;On;2;1;UnityEngine.Rendering.BlendMode;True;0;False;1;1;0;12;0;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;4617;3011.818,291.869;Inherit;False;Property;_DstBlend;DstBlend;51;3;[HideInInspector];[IntRange];[Enum];Create;True;0;3;Default;0;Off;1;On;2;1;UnityEngine.Rendering.BlendMode;True;0;False;1;1;0;12;0;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;4561;3056.885,150.4376;Inherit;False;Property;_DepthWrite;Depth Write;49;1;[Enum];Create;True;0;3;Default;0;Off;1;On;2;0;True;1;Space(5);False;0;0;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.CustomExpressionNode;2275;-4888.854,335.912;Inherit;False;#ifdef STEREO_INSTANCING_ON$return SAMPLE_TEXTURE2D_ARRAY(_CameraNormalsTexture,sampler_CameraNormalsTexture,uvs,unity_StereoEyeIndex)@$#else$return SAMPLE_TEXTURE2D(_CameraNormalsTexture,sampler_CameraNormalsTexture,uvs)@$#endif;4;Create;1;True;uvs;FLOAT2;0,0;In;;Inherit;False;Normal Tex URP;True;False;0;;False;1;0;FLOAT2;0,0;False;1;FLOAT4;0
Node;AmplifyShaderEditor.GetLocalVarNode;2676;1314.259,241.2256;Inherit;False;555;GradientMask;1;0;OBJECT;;False;1;FLOAT;0
Node;AmplifyShaderEditor.GetLocalVarNode;4520;1315.966,172.7616;Inherit;False;4516;ScreenPos;1;0;OBJECT;;False;1;FLOAT2;0
Node;AmplifyShaderEditor.FunctionNode;4793;1494.334,148.2031;Inherit;False;AccurateColors;44;;1923;570575a1eb6bdc7409ed58545512a33b;0;3;12;FLOAT3;0,0,0;False;13;FLOAT2;0,0;False;15;FLOAT;0;False;1;FLOAT3;0
Node;AmplifyShaderEditor.DynamicAppendNode;3578;2986.056,-79.41283;Inherit;False;FLOAT4;4;0;FLOAT3;0,0,0;False;1;FLOAT;0;False;2;FLOAT;0;False;3;FLOAT;1;False;1;FLOAT4;0
Node;AmplifyShaderEditor.TemplateMultiPassMasterNode;4800;3133.681,-81.85668;Float;False;True;-1;2;FPL.CustomMaterialEditor;100;5;LazyEti/BIRP/FakePointLight;0770190933193b94aaa3065e307002fa;True;Unlit;0;0;Unlit;2;True;True;8;5;True;_SrcBlend;1;True;_DstBlend;8;5;False;;1;False;;True;0;False;;0;False;;False;False;False;False;False;False;False;False;False;True;0;False;;True;True;1;False;;False;True;True;True;True;True;0;False;;False;False;False;False;False;False;False;True;False;0;False;;255;False;;255;False;;0;False;;0;False;;0;False;;0;False;;0;False;;0;False;;0;False;;0;False;;True;True;2;True;_DepthWrite;True;7;False;;True;True;1000;False;;2000;False;;True;2;RenderType=Overlay=RenderType;Queue=Overlay=Queue=0;True;3;False;0;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;7;Include;;False;;Native;False;0;0;;Custom;#ifdef STEREO_INSTANCING_ON;False;;Custom;False;0;0;;Custom;UNITY_DECLARE_TEX2DARRAY_NOSAMPLER(_CameraDepthNormalsTexture)@;False;;Custom;False;0;0;;Custom;#else;False;;Custom;False;0;0;;Custom;UNITY_DECLARE_TEX2D_NOSAMPLER(_CameraDepthNormalsTexture)@;False;;Custom;False;0;0;;Custom;#endif;False;;Custom;False;0;0;;Custom;SamplerState sampler_CameraDepthNormalsTexture@;False;;Custom;False;0;0;;;0;0;Standard;1;Vertex Position,InvertActionOnDeselection;1;638685074746738673;0;1;True;False;;True;0
...
WireConnection;4811;69;4528;0
WireConnection;4811;70;4527;0
WireConnection;4811;72;4529;0
WireConnection;4811;74;4531;0
WireConnection;4811;80;4537;0
WireConnection;141;0;4810;0
WireConnection;141;1;143;0
WireConnection;4548;0;4811;0
